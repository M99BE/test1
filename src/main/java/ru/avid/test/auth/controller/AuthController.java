package ru.avid.test.auth.config.controller;
...
import ru.avid.test.auth.entity.Activity;
import ru.avid.test.auth.entity.Role;
import ru.avid.test.auth.entity.User;
import ru.avid.test.auth.exception.RoleNotFoundException;
import ru.avid.test.auth.exception.UserAlreadyActivatedException;
import ru.avid.test.auth.exception.UserOrEmailExistsException;
import ru.avid.test.auth.service.UserDetailsImpl;
import ru.avid.test.auth.service.UserDetailsServiceImpl;
import ru.avid.test.auth.service.UserService;
import ru.avid.test.auth.utils.CookieUtils;
import ru.avid.test.auth.utils.JwtUtils;
...
import static ru.avid.test.auth.service.UserService.DEFAULT_ROLE;
...
//    private EmailService emailService; // сервис отправки писем
    ...
    public ResponseEntity register(@Valid @RequestBody User user) { // здесь параметр user используется, чтобы передать все данные пользователя для регистрации
        ...
        */
        ...
        int updatedCount = userService.activate(activity.getUuid()); // активируем пользователя
